...
def clone_pr_repo(job_obj):
    ...
    pr_repo_loc = repo_dir_str+"/"+repo_name
    ...
    return branch, repo_dir_str, pr_repo_loc
...
def process_logfile(job_obj, logfile):
    ...
    if os.path.exists(logfile):
        ...
        logger.critical(f'Could not find {job_obj.machine["name"]}\
                        .{job_obj.preq_dict["compiler"]} \
                        {job_obj.preq_dict["action"]["name"]} log')
